...
def llm_call(input_file, output_file, prompt_template, start_idx, record_count):
    ...
    with open(input_file, encoding="utf-8") as input_data:
        ...
        for story in json_content:
            ...
            ret_code, err_msg, response = tongyi_tool.call_with_prompt(prompt)
            # NOTE: sleep 1 second, rate limitation TPS <= 60 per minutes:
            time.sleep(1)
            end_time = time.time()
    ...
    with open(input_file, encoding="utf-8") as input_data:
        ...
        for block in json_content:
            ...
            if ret_code == 0:
                ...
                print(ret_code, block["id"], llm_result)
                if ret_code == "DataInspectionFailed":
                    block["answer"] = ""
                else:
                    block["answer"] = "A"
...
if __name__ == "__main__":
    ...
    test_cr_llm_output = "../contest_data/test_cr_tongyi_raw.json"
    llm_call(test_cr_file, test_cr_llm_output, CR_prompt, 0, 1692)
    test_cr_final_output = "../contest_data/test_cr_tongyi_final.json"
    post_process(test_cr_llm_output, test_cr_final_output)

    # test MU set:
    test_mu_file = "../contest_data/test_CRMUS_MU_public.json"
    test_mu_llm_output = "../contest_data/test_mu_tongyi_raw.json"
    llm_call(test_mu_file, test_mu_llm_output, MU_prompt, 0, 1056)
    test_mu_final_output = "../contest_data/test_mu_tongyi_final.json"
